...
aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 073786940416.dkr.ecr.us-east-1.amazonaws.com
```
...
```bash
docker tag todo-app:latest 073786940416.dkr.ecr.us-east-1.amazonaws.com/student1-repo/todo-app:latest
```
...
```bash
docker pull 073786940416.dkr.ecr.us-east-1.amazonaws.com/student1-repo/todo-app:latest
docker run --name todo -dp 80:3000 073786940416.dkr.ecr.us-east-1.amazonaws.com/student1-repo/todo-app:latest
```
- Stop and remove container
...
docker rm todo
```
- make change into the script.`<p className="text-center">No items yet! Add one above!</p>` 
with following new text.

```bash
vi src/static/js/app.js
```
- type '/yet' to find easily the exact place and customize it.
...
docker
```bash
docker tag todo-app:latest 073786940416.dkr.ecr.us-east-2.amazonaws.com/student1-repo/todo-app:latest
```

- Push the image to your ECR repository and show the pushed image on AWS ECR Console.

```bash
docker push 073786940416.dkr.ecr.us-east-1.amazonaws.com/student1-repo/todo-app:latest
```
...
```bash
docker run --name todo-v1 -dp 80:3000 073786940416.dkr.ecr.us-east-1.amazonaws.com/student1-repo/todo-app:latest
```
...
```bash
docker tag todo-app:latest 073786940416.dkr.ecr.us-east-1.amazonaws.com/student2-repo/todo-app:latest
docker push 073786940416.dkr.ecr.us-east-1.amazonaws.com/student2-repo/todo-app:latest
```
- make change into the script.`<p className="text-center">No items yet! Add one above!</p>` 
with following new text.
...
docker push 073786940416.dkr.ecr.us-east-1.amazonaws.com/student2-repo/todo-app:latest
```
- Show the repository . Immutable enable.Change it from the console.

- Push the image to your ECR repository AGAIN and show the pushed image on AWS ECR Console.
...
- Delete the image and repository of `student1-repo/todo-app` from AWS ECR Console manually.
