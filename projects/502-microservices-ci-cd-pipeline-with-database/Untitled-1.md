...
git remote add origin https://github.com/NurulZudin/petclinic-microservices.git
...
> Note: If you get `permission denied` error, try to give execution permission to **mvnw**.  
    ...
    chmod +x mvnw # mo tgk ok ke tak sj
...
git push origin dev
```
dlm step 5 bt yaml file dgn apps automaticaly
## MSP 6 - Prepare Dockerfiles for Microservices # ingat balik Docker dgn dockerfile,dockcompose
sumarize docker, microsevc architecture; we need bt docker images dulu
utk bt docker images kena bt dgn microsvc(dockerfile)
* Create `feature/msp-6` branch from `dev`.

``` bash
git checkout dev
git branch feature/msp-6 # bt branch baru @ git checkout -b feature/msp-6 pastu git branch
git checkout feature/msp-6 #masuk dlm branch tu
```
...
sekarang kita dalam petclini db, petclini admin-server bt file nama Dockerfile; 
``` Dockerfile 
FROM openjdk:11-jre # defined based image, knp guna java env isntead java dev ; kita akn guna runtime env . pbzaan dev dan runtime env ()
pegi docker hub;openjdk - java cmd built our java file , dlm app ada java file p src ada sn utk compile jd perlu java development; developers blh create Dockerfile tp devops pttnya create sbb lps projek redy kita plu deploy projek dgn aws intruktur @ apa2 cloud jd must know cara create Dockerfile. Devops Engineer mest tau Dockerfile; mest compile dlu dgn java compile. dalam target file bila execute mvn clean pckg dr src create target file mcm jar file dan utk executenya xplu openjdk hanya jre;blh execute, compile java src jd kita sdh compile la kiranya dgn jre env. skrg sdh redy dgn env ;guna mvn cln package dlm local env compile java file sdh redi kita akan pkai jre image ok? sbg base image; hr xpkai cont tp guna maven dan compile apps dgn mvn clean packge shj. kita guna ARG instructor
ARG DOCKERIZE_VERSION=v0.6.1 # sama dgn env tp arg bbila buildg image mcm varibale,
ARG EXPOSED_PORT=9090 
ENV SPRING_PROFILES_ACTIVE # docker,mysql, DLM binbash shell ada key=value dlm cont, cth create newenv=itisnew thx env sbb blh pass newenv dlm contaier (docker build && run -it arg-test-6 sh && )knp guna docker env var; dlm container src code ada s=rcources bootsrap.yaml akan profile file bila docker dgn config server klu x; bkn dicker akan build local host sbb ini docker svc akan guna docker cont dan resultnya akan bt spring prof sbg docker thnz to this blh guna container, here bootsrap blh defne profile, other profile adalh mysql; klu xpkai SPRING_PROFILES_ACTIVE adalah deafault value; prtamanya adalah created dgn local host bkn dgn container. dg docker file  profile, blg guna microsvc container dgn docker utk guna app dgn docker container mst SPRING_PROFILES_ACTIVE sbg value jikat x xblg guna app ni dengan dockers. slain tu, mysql sbb defolt value cnfg bila terminate akan hilg smua data t dgn mysql data ok.
ADD https://github.com/jwilder/dockerize/releases/download/${DOCKERIZE_VERSION}/dockerize-alpine-linux-amd64-${DOCKERIZE_VERSION}.tar.gz dockerize.tar.gz # dlm docker.compose ; 
RUN tar -xzf dockerize.tar.gz #xip file sbg continerplu extrct
RUN chmod +x dockerize 
ADD ./target/*.jar /app.jar #bt under target folder jar file; lps bt mvn clean pakcge it wll produce target file jar file dlm zip file to container(target file akan kita create)mcm cp container tu
EXPOSE ${EXPOSED_PORT} #ambk value dr argument 
ENTRYPOINT ["java", "-Djava.security.egd=file:/dev/./urandom","-jar","/app.jar"] executed file # 
# entrypointentrypoint is where you specify command which needs to be run when container is created; assigning arg file other intructions to taking later, firstly, execute app.jar file (mvn clean package java -jar /app.jar; utk execute java file hnay pl); instead using cmd we use entrypoint cthnya, ENTRYPOINT echo "welcome", docker build -t ent, run ent image, it will show welcome. apa bza cmd dgn ent; sbb ent-sbb xmo change defolt comd during execution cth write cmd aan someone change it jd kita mau stiap kali container run this command.ada 2 format 1. exec format 2.shell(tukar dgn json dan)dgn exec xblh call shell;    
```
...
``` bash
./mvnw clean package #ini luar cont lps build bru ada container
docker build --force-rm -t "petclinic-admin-server:dev" ./spring-petclinic-admin-server #take dev sbb dev image;build tis image dgn dockerfile nm blkkg, foce rm alwas remove intermidtae container; bila crt docke image mksdnya create each of layers;cth bt dF
...
docker build --force-rm -t "petclinic-prometheus-server:dev" ./docker/prometheus #created sbb dockerfile
```
...
```bash
./build-dev-docker-images.sh #run dgn sudo docker images; klu tanpa sudo mslhnya adaljh gve eror xblh execute java file
```
...
services: 
  config-server:
    ...
    container_name: config-server #penting utk guna svc name sama image nama sama 
    mem_limit: 512M #instead mem limit ada resources(version3-hnya dgn docker swarm)sbb skrg docker compose maka pakai la mem limit (limit meory utk pass cont;restricted dia)
    ports: 
      - 8888:8888 #dr docker file configserver cth run docker -p 80host:80cont sm jg
  ...
  discovery-server:
    ...
    ports: 
      - 8761:8761
    depends_on: #first creat config server.tgu ready bru blh truskan
      - config-server
    entrypoint: ["./dockerize", "-wait=tcp://config-server:8888", "-timeout=160s", "--", "java", "-Djava.security.egd=file:/dev/./urandom","-jar","/app.jar"] # dockerize utk wait until container is readye blh execute img in order dan excute java comand, dockerize to execute inscredential
