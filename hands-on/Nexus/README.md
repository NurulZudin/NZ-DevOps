...
- Install Java:
Maven is Java based application, so to run Maven we have to install Java on the server. #macven prlu java dlm server
...
```
sudo tar xvzf nexus.tar.gz # unzip
ls
...
```
sudo mv nexus-3* nexus # tukar sbb ada some intergers of nexus, 
ls
```
...
```
ll #owner nya adalah under dir, bila donlod owner and group isthe root, but wen we run nexis as usr we will use amd change grp n user to dir with this two cmd
sudo chown -R ec2-user:ec2-user /opt/nexus
sudo chown -R ec2-user:ec2-user /opt/sonatype-work
ll
```

#pegi cd nexus pastu ls msuk bin edit guna cmd bwh ni dan car nexus.rc user as ec2-user
...
- Another way to run nexus as a service. #selalu run dlm cli klu mau run code, ackage run as service klu stop ec2 so ndak restart auto, atribute dak start nexus lagi tp kita sealu create autiomation
